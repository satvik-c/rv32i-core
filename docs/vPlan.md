...
| **WAIVER - 05** | `Fault Cause` coverpoint (`cp_fault`) merges misaligned and access-fault for loads/stores | `rvfi_mem_addr`/`rvfi_mem_wmask`/`rvfi_mem_rmask` are all zeroed on any trap (`rv32i_core.sv`), so the two causes are indistinguishable from RVFI alone in Stage 1. | Closed by RTL inspection (`lsu.sv`'s `misaligned_access` and the bus `rsp_error` path are distinct and both exercised); `mtval` (Stage 2) is what makes this observable. |
...
| **BUG-02 [RTL]** | Under `RANDOMIZE=1`, `imem` re-fetched the current PC before the in-flight instruction's `dmem` access finished, and could re-trigger a stale duplicate `dmem` transaction after retirement — producing IMEM/DMEM bus mismatches. | `imem_req_valid`/`dmem_req_valid` were driven by each port's own local FSM state plus live decode, with no notion of an instruction already being consumed. Added a registered `instr_valid` flag and gated both requests on it so neither port acts on a stale or already-completed instruction's decode. | `rtl/stall_ctrl.sv` |
| **BUG-03 [RTL]** | Under `RANDOMIZE=1`, `A1` (`rvfi_order` increment check) failed on nearly every retirement, though content/scoreboard checks still passed. | `rvfi_order` incremented the cycle after each retirement instead of on it, so its new value became visible during the idle gap before the next retirement rather than at it — invisible under `RANDOMIZE=0` since back-to-back retirements have no gap for the lag to show in. Split into a live `retired_count` (this retirement's order) and a `last_order` register held between retirements, muxed by `rvfi_valid`. | `rtl/rv32i_core.sv` |
| **BUG-04 [RTL]** | Under `RANDOMIZE=1`, `test_compiled_c` halted early with no `SIM_EXIT`, only reproducible with compiled code. | `misaligned_access`/`misaligned_fetch` decode combinationally off `instr_word`, which holds the just-retired instruction until the next fetch response arrives. An instruction whose `rd` aliased its own `rs1` could re-decode against the regfile's already-updated value in that gap and trip a phantom fault. Added `decode_valid`, gating `trap_active` to instructions still live. | `rtl/stall_ctrl.sv`, `rtl/fault_ctrl.sv`, `rtl/rv32i_core.sv` |
| **BUG-05 [RTL, formal]** | `riscv-formal` `insn_*` checks: a cleanly-retired instruction was immediately followed by a second, falsely-trapping retirement of the same stale bits. | `instr_word`'s error fallback didn't distinguish a live in-flight retry from an already-retired instruction's stale bits, and `fault.imem_error`/`dmem_error` ignored `rsp_valid`. Gated the fallback on `instr_valid` and the faults on `rsp_valid`. | `rtl/rv32i_core.sv`, `rtl/stall_ctrl.sv` |
| **BUG-06 [RTL, formal]** | `riscv-formal` `insn_*` checks: `rvfi_rs2_addr` always reported `x0` for R-type, and `rvfi_rd_addr` was nonzero for `FENCE`. | `main_dec` never set `imm_src` for R-type, defaulting to `IMM_I` — a value the rs1/rs2/rd-write heuristic reads as "no rs2". Added `IMM_R`/`IMM_NONE` enum values for R-type and no-register formats. | `rtl/core_pkg.sv`, `rtl/main_dec.sv`, `rtl/rv32i_core.sv` |
